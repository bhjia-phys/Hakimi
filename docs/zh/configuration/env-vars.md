...
Hakimi 通过环境变量控制少数运行时行为——迁移数据目录、关闭遥测、不改配置文件临时切换模型。
...
覆盖数据根目录，默认 `~/.hakimi`。设置后，配置文件、会话、日志、OAuth 凭据等全部数据都落到新路径下：

```sh
export KIMI_CODE_HOME="/path/to/custom/hakimi"
```

> 如果设置了 `HAKIMI_HOME`，则其优先级最高，其次为 `KIMI_CODE_HOME`，最后是默认的 `~/.hakimi`。确保目录可写。多个 `hakimi` 实例共用同一个数据根目录会共享配置和凭证。
...
| `KIMI_CODE_EXPERIMENTAL_AUTO_SUBAGENT_PRESET` | 启用加权的自动 subagent preset 切换：在相关 Agent、AgentSwarm 或 Tower 派生及允许重绑定的 resume 前，引擎会按 provider 配额 / reset、本地优先级、profile-aware 可靠性与首 token 延迟、token 用量和路由 / 模型匹配度为候选评分，再激活得分最高的健康 `[subagent].preset`。还要求在 `[subagent.auto_preset]` 下设置 `enabled = true`；默认关闭，只写 `[subagent].preset`，不训练模型、不在空闲时轮询，也绝不会阻塞派生或 resume。master `KIMI_CODE_EXPERIMENTAL_FLAG=1` 也会启用本功能 | 真值：`1`/`true`/`yes`/`on`；假值：`0`/`false`/`no`/`off` |
| `KIMI_CODE_EXPERIMENTAL_REMOTE_CONTROL` | Hakimi Web 远程控制默认开启，可以通过短期 Cloudflare Quick Tunnel 查看、切换和控制所有现有会话；把本变量设为假值可关闭 Web 控制路由。TUI 的 `/remote` 命令始终可用，不读取本变量。见[从其他设备远程控制](../guides/sessions.md#从其他设备远程控制) | 假值 `0`/`false`/`no`/`off` 关闭 Web 入口；其他情况保持开启 |
| `KIMI_CODE_CLOUDFLARED_PATH` | `PATH` 中找不到 `cloudflared` 时，Web 远程控制、TUI `/remote` 和 `hakimi remote start` 使用的 `cloudflared` 可执行文件绝对路径；显式传入 `--cloudflared` 时优先使用该选项 | 可执行文件的绝对路径 |
...
`KIMI_CODE_IDENTITY_*` 和 `KIMI_CODE_BUILTIN_PRODUCT_SKILLS` 这三个变量由默认的 `agent-core-v2` 引擎读取。设置 `KIMI_CODE_LEGACY_FLAG=1` 后，旧版 `hakimi` / `hakimi -p` 路径会忽略它们。
...
Hakimi 会遵循标准代理环境变量，让所有出网流量——模型 API 调用、MCP 服务、网络工具、遥测、登录、更新检查——都走代理：
...
以 Node 子进程运行的 stdio MCP 服务，在其 Node 版本支持 `NODE_USE_ENV_PROXY` 时（Node ≥ 22.21 或 ≥ 24.5）会自动遵循 `HTTP_PROXY` / `HTTPS_PROXY` / `NO_PROXY`；SOCKS 代理仅作用于 Hakimi 自身的流量。
