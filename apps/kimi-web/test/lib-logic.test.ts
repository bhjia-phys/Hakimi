...
describe('filePathLinks', () => {
  ...
  describe('hrefToFilePath', () => {
    it('decodes a percent-encoded non-ASCII filename', () => {
      expect(hrefToFilePath('ppt/example_%E5%8D%95%E5%9B%BE%E5%B7%A5%E7%A8%8B.pptx')).toBe(
        'ppt/example_单图工程.pptx',
      );
      expect(hrefToFilePath('a%20b.txt')).toBe('a b.txt');
    });

    it('strips ?query and #fragment before decoding', () => {
      expect(hrefToFilePath('docs/a.md?raw=1#L12')).toBe('docs/a.md');
      expect(hrefToFilePath('docs/a.md#L12')).toBe('docs/a.md');
    });

    it('keeps an encoded %23 / %3F as a literal filename character', () => {
      expect(hrefToFilePath('ppt/a%23b.pptx')).toBe('ppt/a#b.pptx');
      expect(hrefToFilePath('a%3Fb.txt')).toBe('a?b.txt');
    });

    it('decodes exactly once — a literal %20 sequence stays encoded', () => {
      expect(hrefToFilePath('a%2520b.txt')).toBe('a%20b.txt');
    });

    it('passes a plain path through unchanged', () => {
      expect(hrefToFilePath('src/a.ts')).toBe('src/a.ts');
    });

    it('never throws on a malformed percent sequence', () => {
      expect(hrefToFilePath('a%zz.txt')).toBe('a%zz.txt');
      expect(hrefToFilePath('a%2.txt')).toBe('a%2.txt');
      expect(hrefToFilePath('100%.txt')).toBe('100%.txt');
    });

    it('surfaces a decoded traversal for the downstream workspace validation', () => {
      expect(hrefToFilePath('..%2Fsecret.txt')).toBe('../secret.txt');
      expect(hrefToFilePath('..%2F..%2Fetc/passwd')).toBe('../../etc/passwd');
    });
  });
});
