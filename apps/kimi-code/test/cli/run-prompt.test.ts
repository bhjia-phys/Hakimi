  ...
  return {
    ...
    runV2Print: vi.fn(
      async (
        ...
        if (outputFormat === 'stream-json') {
          ...
          stdout.write(
            `${JSON.stringify({
              ...
              command: 'hakimi --session ses_prompt',
              content: 'To resume this session: hakimi --session ses_prompt',
            })}\n`,
          );
          return;
        }
        ...
        stderr.write('To resume this session: hakimi --session ses_prompt\n');
      },
    ),
...
describe('runPrompt', () => {
  ...
  it('emits the version first in text mode on the default v2 engine', async () => {
    ...
    expect(stderr.text().startsWith('hakimi version 1.2.3-test\n')).toBe(true);
    expect(stdout.text()).toBe('• hello world\n\n');
  });
  ...
  it('uses the legacy engine when legacy wins over the experimental flag', async () => {
    ...
    expect(stderr.text()).not.toContain('hakimi version');
  });
