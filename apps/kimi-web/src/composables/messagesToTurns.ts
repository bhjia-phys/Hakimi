...
/** Continuous text of a toolUse part, with explicit semantics:
 *  - prefer the reducer's byte-faithful `outputText` (verbatim chunk
 *    concatenation);
 *  - otherwise '\n'-join line-shaped `outputLines` (seeded/restored forms
 *    carry COMPLETE lines, so the join reconstructs the exact stream and
 *    cannot glue adjacent lines together). */
function toolUseOutputText(c: {
  outputText?: string;
  outputLines?: string[];
}): string | undefined {
  if (c.outputText !== undefined) return c.outputText;
  return c.outputLines && c.outputLines.length > 0 ? c.outputLines.join('\n') : undefined;
}

/** Continuous text of a final tool result: a string output is verbatim; a
 *  ContentPart[] is flattened the same way normalizeToolOutput does, then
 *  '\n'-joined. Returns undefined only when there is no output at all. */
function toolResultOutputText(output: unknown): string | undefined {
  if (typeof output === 'string') return output;
  const lines = normalizeToolOutput(output);
    ...
    for (const c of content) {
      if (c.type === 'text') {
        ...
        if (idx !== -1) {
          ...
          const finalOutputText = toolResultOutputText(c.output);
          const updated: ToolCall = {
            ...tool,
            status: c.isError ? 'error' : 'ok',
            output: normalizeToolOutput(c.output),
            // The result is authoritative once it arrives — except when it
            // carries no output at all, in which case the accumulated streamed
            // text is kept instead of being wiped.
            outputText: finalOutputText !== undefined ? finalOutputText : tool.outputText,
  ...
  function mergeVolatileExtras(g: Group, content: AppMessage['content']): void {
    for (const c of content) {
      ...
      const updated: ToolCall = {
        ...tool,
        output: c.outputLines,
        // Carry the seed's continuous text too, but never overwrite a text
        // the already-mapped toolUse provided.
        outputText: tool.outputText ?? toolUseOutputText(c),
      };
      g.tools[idx] = updated;
      const blk = g.blocks.find((b) => b.kind === 'tool' && b.tool.id === c.toolCallId);
      if (blk && blk.kind === 'tool') blk.tool = updated;
