...
import { toAppMessageContent, toAppResearchModeSnapshot } from './mappers';
import type { WireMessageContent, WireResearchModeSnapshot } from './wire';
...
function appendToolUse(
  ...
  const msg = state.messages.find((m) => m.id === messageId);
  if (!msg) return;
  msg.content.push({ type: 'toolUse', toolCallId, toolName, input, outputLines, outputText });
}
...
export function createAgentProjector(): AgentProjector {
  ...
  function seedInFlight(sessionId: string, turn: AppInFlightTurn): AppEvent[] {
    ...
    for (const tool of turn.runningTools) {
      // The seeded snapshot only carries a final progress text — a raw stream
      // string, NOT line-split — so it seeds BOTH the line-shaped `outputLines`
      // and the continuous `outputText` with the same explicit semantics.
      ...
      msg.content.push({
        ...
        toolName: tool.name,
        input: tool.args ?? {},
        outputLines: lastProgressText !== undefined ? [lastProgressText] : undefined,
        outputText: lastProgressText,
      });
    ...
    switch (rawType) {
      ...
      case 'research_mode.updated': {
        ...
        out.push({
          ...
          sessionId,
          snapshot: toAppResearchModeSnapshot(p.snapshot as WireResearchModeSnapshot),
        });
        break;
      }
