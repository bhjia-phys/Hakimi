...
import WebPreviewCard from './WebPreviewCard.vue';
...
const props = withDefaults(
  defineProps<{
    ...
    isFollowing?: boolean;
    /**
     * When true, the standalone preview card (artifact file/media or local
     * dev-server web fallback) is rendered at the end of each assistant reply.
     * Off in compact contexts that must not offer a main action inside the
     * right-side panel (e.g. the BTW side chat), where opening the file /
     * media / preview panel would only replace the panel already hosting this
     * transcript.
     */
...
const emit = defineEmits<{
  ...
  /** Open a local dev-server URL (detected in a tool's output) in the
   *  right-side web preview panel. */
...
/** The turn's single preview target — an artifact (written/edited file or
 *  produced media) wins over a dev-server URL from tool output; computed once
 *  per turns change and keyed by turn id for the template branches below. */
const previewTargets = computed<Map<string, TurnPreviewTarget | null>>(() => {
  const map = new Map<string, TurnPreviewTarget | null>();
  for (const turn of props.turns) {
    map.set(turn.id, turn.role === 'assistant' ? findTurnPreviewTarget(turn.tools) : null);
  }
  return map;
});

/** The artifact subset of the turn's target — null for web/none, so the two
 *  card branches below stay mutually exclusive (one card per turn max). */
function artifactPreviewTarget(turn: ChatTurn): ArtifactPreviewTarget | null {
  const target = previewTargets.value.get(turn.id);
  return target && target.kind !== 'web' ? target : null;
}

/** The web-fallback subset — the url only, per the WebPreviewCard contract. */
function webPreviewTarget(turn: ChatTurn): WebPreviewTarget | null {
  const target = previewTargets.value.get(turn.id);
  return target && target.kind === 'web' ? { url: target.url } : null;
}
    ...
    <template v-for="(turn, ti) in turns" :key="turn.id">
      ...
      <div v-else class="a-msg turn-anchor" :data-turn-id="turn.id">
        ...
        <!-- Standalone preview card: the turn's ONE preview target — an
             ...
             media) wins over a dev-server URL printed by a tool. At most one
             card per turn — after all content blocks, before the turn footer. -->
        <ArtifactPreviewCard
          v-if="previewCards && artifactPreviewTarget(turn)"
          :target="artifactPreviewTarget(turn)!"
          @open-file="emit('openFile', $event)"
          @open-media="emit('openMedia', $event)"
        />
        <WebPreviewCard
          v-else-if="previewCards && webPreviewTarget(turn)"
          :target="webPreviewTarget(turn)!"
          @open-preview="emit('openPreview', $event)"
        />
