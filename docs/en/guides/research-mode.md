...
If the original scope was not captured, review the retained result in Web's Checkpoint section and choose **Confirm ownership and prepare checkpoint**, or use `/research adopt-conclusion <localConclusionId> <lineSlug> [questionId]`. Confirm the target Line's workstream first. Manual adoption remains available for explicit recovery; it is not the default step for already confirmed original ownership. Human-authority conclusions, stale context, paused/degraded mode and possible external commits do not qualify for automatic proposal. Stable checkpoint and idempotency keys are reused after undo; any committed-history entry at or after the conclusion keeps automatic recovery closed pending an explicit evidence review.
...
After submission persistence, `BeginResearchAction` can set `observed_run_action_id` to the retained `currentRun.actionId`, preserving the same Question and Line. The new observation Action retains that Run and its original submission identity; `ObserveResearchRun` takes the new foreground Action ID but does not reassign the Run origin. Other replacement Actions remain blocked while the Run is live. Ordinary Action tool permissions still apply; a shell grant is not an OS-enforced read-only job query. No scheduler polling, new submission, or automatic resumption of a blocked Goal is added.

### Evidence reads while recording
