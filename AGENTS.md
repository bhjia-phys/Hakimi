...
- `apps/kimi-web` is the browser Web UI and the only editable production source for the tracked derived outputs `apps/kimi-code/dist-web` and `apps/kimi-code/web-base.json`; develop it with `pnpm dev:server` plus `KIMI_SERVER_URL=http://127.0.0.1:58627 pnpm dev:web`. After source changes, regenerate both package assets with `pnpm run build:web-assets`, commit them together, then verify a clean byte-for-byte rebuild with `pnpm run build:web-assets -- --check` — never hand-edit or partially replace the outputs; CI, native, and release flows verify tracked outputs before regenerating them, while direct package build/prepack and Nix generate and validate outputs for consumption, schema v4 binds source/recipe/bundle identity, the native receipt also binds the final binary hash, and rollback restores source/recipe from a canonical tag before regeneration.
