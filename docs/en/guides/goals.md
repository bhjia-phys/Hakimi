...
Hakimi saves the objective, sends it as the next user message, and starts goal mode. After each turn, it checks whether the goal is complete, blocked, paused, or still active.
...
1. Use goals for work with a clear finish line and verifiable evidence.
    ...
    Hakimi can inspect test output, change files, rerun checks, and decide when the goal is complete.
...
- **blocked**: Hakimi needs input, cannot complete the goal as stated, or reached a budget limit. When the agent blocks a goal, it writes a short message explaining why.
...
`active` describes the Goal lifecycle; it does not claim that a model turn is running at every instant. On the default v2 engine, Goal snapshots may also expose transient continuation detail such as a background-task wait lease. The lightweight Research Mode architecture does not add a Research-specific completion or continuation veto, and ordinary Goal behavior, Plan mode, and tool permissions remain unchanged. The lightweight Research Mode is locally implemented and installed; newly started Hakimi processes use the built workspace output, while already-running processes need a restart. It adds no Research-specific completion or continuation veto, and ordinary Goal behavior, Plan mode, and tool permissions remain unchanged. Historical notes about Research Loops, Actions, alignment gates, and Research Boards do not describe the new backend.
...
Upcoming goals are not visible to the agent while the current goal is running. When the current goal completes, Hakimi starts the first upcoming goal in the same way as users enter `/goal <objective>`.
...
If the current goal is paused, canceled, or blocked, Hakimi does not start the next upcoming goal. When a goal blocks and upcoming goals exist, the TUI reminds you that they wait for completion.
