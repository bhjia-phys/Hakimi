...
A further run-observation recovery fix is under validation: a closed Action can record a fresh observation of its existing external job without reopening the Action or changing its conclusion. This grants no polling or new-work permission. See the [bounded recovery slice](docs/aitp/theory-physics-collaborator-program.md#retained-run-recovery) for evidence and remaining installation checks.
