...
export class OpenAILegacyChatProvider implements ChatProvider {
  ...
  async generate(
    ...
    // Determine reasoning_effort. DeepSeek maps enabled efforts to its
    // documented high/max values. Other compatible endpoints receive concrete
    // efforts verbatim. 'off' sends a model-declared wire value (for example
    // 'none') when one is configured.
