...
import ProviderUsageDialog from './settings/ProviderUsageDialog.vue';
...
const { t } = useI18n();
const showProviderUsage = ref(false);
...
<template>
  ...
  >
    ...
    <div class="col" :style="{ width: colWidth + 'px' }">
      ...
      <div class="ch">
        <div class="ch-brand">
          <template v-if="!isMacosDesktop">
            <svg ref="logoRef" class="ch-logo" :class="{ 'is-dev': isDev }" viewBox="0 0 32 22" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Hakimi" @click="onLogoClick" @pointerdown="onLogoPointerDown" @pointerup="onLogoPointerUp" @pointercancel="onLogoPointerUp">
              <defs>
              ...
              <path d="M2 8L4 2L10 5C12 4 20 4 22 5L28 2L30 8V15C30 19 26 21 16 21C6 21 2 19 2 15V8Z" fill="var(--logo)" mask="url(#kimiEyes)" />
            </svg>
            <span class="ch-name">Hakimi</span>
            <Pill
              v-if="isDev"
              class="ch-backend"
      ...
      <!-- Footer: quick usage and settings entries pinned under the session list -->
      <div class="side-footer">
        <button
          class="btn-settings"
          type="button"
          :aria-label="t('sidebar.providerUsage')"
          @click.stop="showProviderUsage = true"
        >
          <Icon name="sliders" />
          <span>{{ t('sidebar.providerUsage') }}</span>
        </button>
    ...
    />
    <ProviderUsageDialog
      v-if="showProviderUsage"
      @close="showProviderUsage = false"
    />
...
/* Footer — quick usage and settings entries pinned under the session list.
   Same list-style control family as search / New chat (full-width,
   left-aligned, hover sunken — not a Button). */
.side-footer {
  flex: none;
