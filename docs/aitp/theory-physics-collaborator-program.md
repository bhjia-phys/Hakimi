...
当前 Goal 的续行顺序与验收收敛方式见 [§19.24](#goal-execution-refinement)，已有证据和未完成项见 [§19.26](#acceptance-evidence-matrix)。用户取消旧 Goal 后，新 Goal 完整继承这里的范围和证据；不重复已完成的旧下一步，不清空失败与未测，也不表示功能已全部验收。
...
本节把总 Goal 收敛为可逐项验收的交付任务，不改变 G0–G7 的范围或 C1–C5 的独立需求条件。Goal 的 objective 仍是交付首个可用的串行理论物理合作者，并在指定 Heisenberg 课题完成一个有限、可复查的科学里程碑；不是完成全部开放物理问题，也不是把每个候选平台能力实现一遍。

**继承依据（2026-09-05 更新）。** 用户明确取消旧 Goal 后要求优化重设，宿主查询确认旧 Goal 已不存在；新 Goal 继承全部范围、已完成交付、失败和未完成证据，不并行保留两个目标，也不把旧 Goal 假标 complete。旧正文里的“当前修复尚未交付”等表述只是当时快照；后续以本文件最新验收记录与当前代码、安装回执为准，不重做 §19.23、§19.25、§19.27、§19.32 已交付且证据充分的部分。只有受后续修改影响的路径才安排回归。
...
**新 Goal 的实际优先级。** simple Action 关联与 Question 上下文修复已交付安装，不再是待开发项。§19.34 证明单 turn 有限 Goal 收尾；[§19.36](#goal-continuation-exit-review) 新增一次 `/goal resume` 后三个真实自动 continuation turns，但整次运行在外部时限退出，退出前 journal 仍保留 active Goal 和未收尾 Action。该 TERM 退出竞争已在 [§19.37](#tui-sigterm-shutdown) 修复、交付安装并独立进程复测。[§19.38](#goal-continuation-artifact-audit) 已独立核对 r3 的六份产物、Entry 和冷恢复，确认数值可用但矩阵/笔记持久化与计划同步未完成；现在只做必要恢复收尾，不重复计算或证明“能发起下一 turn”。SIGHUP、idle/held Goal 关闭及计划漂移仍保留，不因本次 TERM 修复关闭 G2。正常科研提示不指定内部工具或 phase；不能靠人为拆 turn、控制器代为完成 Goal，或 cold restore 补暂停获得通过。Heisenberg 正式归属沿用已有确认，缺失部分保持明确依赖，不以 fixture 模拟确认替代。
...
| 用户暂停/取消、科学约定/资源/范围变化时合理停止 | G pause/cancel/budget/restore；R Goal guard/human gate | print 信号退出有 §19.32 的独立进程证据；E12 的 TERM 竞争已在 §19.37 交付修复，安装 TUI 在 cold restore 前保存暂停和取消。SIGHUP、idle/held 关闭及完整继续→等待/恢复→完成或必要提问仍未验收；Heisenberg 归属依赖不由测试代签 |
...
另一个真实摩擦是首次 simple Begin 带总计划 milestone 被拒绝：`A simple Research action cannot carry reviewed-plan bindings.` 去掉绑定后才成功，没有工具绕过，但丢失了显式 milestone 关联。现有 freshness 检查已能分别检查 parent Research Plan 与 local Action Plan；最小设计是解耦“属于哪个里程碑”和“是否需要详细执行计划”，不新增字段或替人审批。本次运行时尚未修复；后续实现与验证见 §19.33。G2/D/G7 仍未通过，不增加计算或延长上限来掩盖收尾失败。
...
文档 allowlist：双语根 README、双语 `docs/*/reference/kimi-command.md`、Hakimi `docs/aitp/README.md`/`compatibility-matrix.md`、本文件，以及 `.changeset/print-goal-shutdown.md`（仅 CLI patch）。原 Hakimi checkout、AITP 的 20 条 dirty paths 和真实科研项目不改；SDK、REST/WS、klient、TUI/Web schema 与生产资产不变。当前交付状态：`49f51fbdc20b76a21ebc74af1df5331ad957b64e` 已推送到开发分支并从干净提交构建、安装，安装版信号复测通过。
...
安装与独立进程验收（2026-09-05）：干净构建的 packages、CLI、smoke、docs 和 pack 均 exit 0；安装入口 SHA256 为 `58d1aaa9305c1df8b6d9b15013a74aadf1ee4ab9d406788d478cedcab30dab22`，与构建一致，521 个 Web 文件和 provenance 逐字节一致，CLI help/version 与真实 PTY 通过。版本仍为 0.21.0，未消费累计 changesets。首次 npm 安装拦截 `node-pty` 脚本导致 PTY 模块缺失；project-scoped rebuild 又因 `EALLOWSCRIPTS` 拒绝。随后仅通过一次 global install 的 `--allow-scripts=node-pty` 完成该依赖原生构建，未修改全局配置；旧安装包保留。回执为 `/tmp/hakimi-print-shutdown-install.JhySZk/installation-verification.json`。

三个独立进程只连接一次性本地未响应 HTTP provider，不调用真实模型、不打开科研项目，也不先执行 SDK cold restore。SIGINT/TERM/HUP 分别退出 130/143/129，清理耗时 71/121/107 ms；磁盘 journal 均在退出时已有 `Paused when print mode exited` 和 cancelled turn，没有 `Paused after agent resume`。原始回执在 `/tmp/hakimi-print-signal-NIehzX/acceptance.json`，每例保留 stdout/stderr/wire。此项证明安装版正常信号退出链，不能证明 OS 隔离、SIGKILL 保存或跨 turn 科研完成。§19.31 的原超时保留为失败；下一项仍是 simple Action 的总计划 milestone 关联与真实 Goal continuation。

### 19.33 小检验关联总计划里程碑 {#simple-action-milestone}

2026-09-05，用户取消旧 Goal 后，新 Goal 完整继承 G0–G7/C1–C5 和已有失败/未测；本节是该 Goal 的首个实现切片，不重开或缩减总目标。依据 §19.31 的真实拒绝，Hakimi 将“属于当前哪个里程碑”与“是否需要 reviewed local Action Plan”解耦：`simple`（含省略 planning level）可显式提供完整的 parent ID/revision/milestone，同时保留原 minimal binding；不提供则不推断归属。`planned` 的 reviewed local Plan 和已有 parent 规则不变。

复用既有 freshness 与 executor 检查，不新增 API、字段、phase、服务或工具能力。模型输入拒绝不完整的 parent triple 和 simple 的 reviewed local-plan 字段；服务拒绝错误/非 active/过期 parent。已绑定 action 存在时 parent 不可被服务自身修订或终止，执行前和收尾仍校验 parent。Question/Line 更新撤销旧执行许可、拒绝新的 durable conclusion；既有显式 no-delta 放弃路径保留，不把仅本地收尾错误地禁死。未绑定 AITP workstream 的新结果仍留作 local conclusion，不自动写入 AITP、完成 Plan 或 Goal。

本切片的 12 文件范围：两个 core 实现文件、Research service/协议/SDK 三个测试文件、README 双语、`docs/aitp/` 四个既有文档，以及 `.changeset/simple-research-milestone.md`（仅 CLI patch）。原 Hakimi checkout `892733a00582` 的 156 条 dirty paths 和 AITP `eae1bce5eba3` 的 20 条 dirty paths 未动，科研目录只保留既有证据，不新跑计算。

源码验证：单 worker 的 Research service 589、ops/policy/transition/Goal 五文件 312、protocol 46、REST/WS 32、SDK 定向转发 1、klient 定向转发 1、TUI 148、Web 39，共 1,168 项通过。新增用例覆盖 minimal+parent 同时保留、部分/错误/过期关联、执行撤权、durable 拒绝、no-delta 放弃、local result 与幂等、undo/replay；JSON round-trip 与 SDK 转发有明确 simple+parent case。REST/klient/TUI/Web 的其余结果是既有路径回归及代码投影核验，不冒充本场景的安装版端到端测试。

首次类型检查发现可选字段收窄和 fixture readonly 数组问题，已修正；最初两项测试误将 Question/Line 更新后的 no-delta 收尾也视为非法，按代码既有边界改为检验 durable 拒绝，并另补显式 no-delta 放弃。SDK 首次 5 秒超时；单独重跑仍使用原 5 秒上限后通过，首次失败保留，未宣称超时根因已证实。core/protocol/SDK/klient（含 examples）typecheck 与 1,299-file import guard 通过。五个修改的 TS 文件单线程 type-aware lint 为 0 errors、87 warnings，新增修改区间无 warning；无关旧 warning 未清理。CLI patch 不消费已有 changesets，不新增 SDK bump。
...
### 19.34 第三点判别、完整收尾与上下文缺口 {#third-observation-acceptance}

2026-09-05 使用已安装 `f6487d990` CLI 0.21.0、Theory Physics 0.2.3、AITP 0.9.0/contract-0.2，在既有隔离 H2O Topic 运行 `goal-plan-r2`，模型沿用 `openai-codex/gpt-6-astra` / high。控制器仅通过公开 SDK 建立该测试 session 的范围、暂停的有限 Goal 与 dreaming/auto；不是 Heisenberg 或人类科学决定。自然提示给出第三点问题、两个条件性算例、简短可修订计划要求和 600 秒上限，不指定内部工具、phase、Action 类型、拆 turn 或控制器完成 Goal。

模型先一次 no-delta Action 回读已有依据、确认空卡检索和新目录，再创建单里程碑 Plan，自行选择 `simple` 推导 Action 并显式关联该里程碑。新结论为非负平台单指数族的必要充分区间 `b²/a <= c < b`、唯一反解 `q=(b-c)/(a-b)` / `L=(ac-b²)/(a-2b+c)`，及边界、全输入梯度与有限误差说明。标准库 Fraction 检验两个算例、正负扰动和八角点，计算约 0.0113 秒，受 30 秒/512 MiB 上限约束；没有第三次 QSGW 数据或新 LibRPA 运行。独立 JavaScript 使用闭式参数、代回和扰动比较 62 项，最大绝对差 `3.068265720706023e-14`；另复核必要充分性、退化和有限前缀不能决定尾极限的论证。一次审计脚本的字符串转数值遗漏在控制台产生无效 midpoint，已排除并改正后才验证，不当作产品故障或通过证据。

本次仅新增 `data/goal-plan-r2/` 的推导、脚本、结果三个文件和 canonical `entry-ae81dfa1822d4deda51c272ba4bc0914`；保护清单中 70 个旧文件不变，不涵盖旧展开 source/build 树，不声称 OS sandbox。模型正常 Begin/work/Conclude，随后官方 prepare/fill/save/show/check/commit；Question 最终 revision 4、closed/supported/committed，Plan revision 3 completed，无 pending/live action。wire 记录模型 `goal.update status=complete`、`goal.clear` 和唯一 user turn 的正常结束；独立恢复无 active Goal，exit 0，总时长 573.913 秒，未触发外部中断。模型所报约 8m41s 是激活后的 Goal 时长，不等于 CLI 总时长。官方 scoped check 为 8 Entries、1 Note、0 errors/warnings，scope 外原有一条 warning 保留；没有新 Note/card/trial/human decision 或旧 failure resolution。仅一个 user turn，不证明跨 turn 自动续行，也不把提示中禁止造卡当作自主蒸馏能力的证明。

**保留的缺口。** 保存前 Question revision 1 经 candidate/commit 变为 3；模型用旧 revision 更新被拒绝，查询后才成功。`CommitResearchCheckpointTool.questionSynthesisGuidance()` 已有同轮提示，但只读取 `snapshot.currentQuestion`，后者仅从显式 Focus 派生。本次 Action/checkpoint 有准确 Question ID，却没有单独 SetResearchFocus，因此提示空缺。最终 snapshot 73 的 Question 已明确写明停止，但 `deriveEffectiveNextStep()` 仍先取旧 `latestProgress.nextAction`，显示“保存核验、完成计划与 Goal”，且 `derivedFrom` 为空。科研结果和退出成功不掩盖这个 Board 上下文/next 缺陷；单纯将 warning 清零不能修复它。

**下一唯一最小 Action。** 用同一真实形状补充回归，修复显式 action/checkpoint Question 的回执与 next 投影，不自动设置/覆盖 Focus，不挑选未指定 Question，不把相同 slug 当绑定，不弱化 stale/receipt/跨 Line 检查。已由模型明确更新的当前结论不得再被历史 progress next 覆盖；无法证明当前归属时保留不确定，不推断完成或放弃。允许修改仅限相关 core projection/tool、已有 tests、必要六端投影验证、双语操作说明与 CLI patch changeset；无新 API/schema/服务/AITP 行为。先验证有/无 Focus、不同 Focus、不同 Line、重复 commit、pending、较新 revision 和冷恢复，再交付及复测受影响路径，不重算本次代数来证明文案修复。真实跨 turn 与正式 Heisenberg 里程碑仍未完成。

原始证据在隔离 worktree 的 `.tmp/research-acceptance-20260905/goal-plan-r2/`、相邻 before/after JSON 和实际 session `session_bbfc47cb-c806-4bd3-bccf-fbe03c9f553a`；包括提示、完整输出/错误流、运行时间片 wire、资源限制和独立核验。原 Hakimi HEAD `892733a005` 的 156 个 dirty paths、AITP HEAD `eae1bce5eb` 的 20 个 dirty paths 仍保留；三个用户进程未关闭。Heisenberg 只读 `enter/check/show` 确认最新记录仍为 9 月 1 日，验收 session 无 Goal/Lines/绑定；其历史 24 个 hash errors、263 warnings 没有被改写。唯一旧 method-observation 属于其他研究线，本轮无新候选基础，遵守 Skill 的 no-op，不全库蒸馏或修改科研 ledger。
...
允许修改为这一共享 core 投影、对应已有测试、TUI 的证据标题及测试、Web 中英证据标题及已有投影测试、双语 README、本侧三份 handoff、本文件、CLI patch changeset，以及官方流程重建的 Web assets/provenance。无公共字段、API、AITP runtime/CLI/schema/contract/Skill 或人类决定语义变化；原 Hakimi checkout、AITP 的 20 项 dirty changes、科研目录和用户进程保持不动。借助 `agent-core-dev` 的 DI 测试路径验证独立实例恢复；`write-tui` 保持展示层无状态写入；`gen-changesets` 仅选择 CLI patch。

源码验证包括有/无 Focus 的 prepare/save/show/check/commit 全链，显式其他 Focus、当前／其他 Line、无 Action／未指定 Question、planned／in-progress／completed、blocked Question 和独立容器 journal 恢复。Question 显式综合后，历史 progress 保留，snapshot 与 status next 一致；重复提交、其他 checkpoint、更新的 revision、paused/degraded 等原有定向提示抑制测试保留。TUI/Web 将详细证据标题改为 “Current-question evidence / 当前问题证据”，避免把 Action 上下文误称为 Focus。

初次测试错误保留：新增断言误读 checkpointed Model 的外层而非 `.current`；试图强行切走 live Action 被现有恢复纠正、从 state_updated 跨 Line 设置 Focus 被合法拒绝，因此隔离用例改为真实完成后切线和显式旧状态投影；Web 用例遗漏 `questions` 内的当前问题，被既有 provenance 过滤，补齐合法 snapshot 而非放宽 UI 判断。首轮 core 相关 1,123 项、类型检查及 1,299-file import guard 通过，TUI 92 项、Web 40 项、protocol 46 项通过。REST/WS 首轮 28 通过、4 个测试及 teardown 超时；首个失败在原 5 秒上限单独重跑通过，但尚不足以说明整文件健康。初次 Web 构建与该接口测试误有短暂重叠，未发生 OOM；构建已结束，后续验证严格等待当前句柄结束后串行。未扩大超时、修改无关测试或声称失败根因已经证实。后续验收结果追加在此。

最终源码回归：core 17 文件 1,123 项通过；REST/WS 在原超时、无其他构建并行时整文件 32 项通过；SDK 定向 Research 18 项通过（43 项不在筛选范围），klient 定向 19 项通过（34 项不在筛选范围）。TUI 92、Web 40、protocol 46 均通过。core/CLI/Web typecheck、import guard、文档构建（11.19 秒）、521 个 Web 资产的 canonical rebuild/`--check`、`git diff --check` 通过。修改的 7 个 TS 文件单线程 type-aware lint 为 86 warnings / 0 errors，diff 行号核验未发现新增修改行警告；Web style 为 baseline 模式的 28 个既有 findings，不称零警告。AITP 官方 `test_adapter_contract.py` 与 `test_atomic_record_save.py` 为 21 passed；无 AITP 文件变动。changeset 的初次 `--since=HEAD` 检查尚未识别 untracked 新文件，需在精确暂存后复查，不能据此添加空 changeset。
...
**交付与安装。** `74743f2594a217ee31f859112694d110b0dc1f4d` 已提交、推送到既有 `merge/pr-9-auto-subagent-preset` 分支。干净源码来自该提交的 `git archive`，在 `/tmp/hakimi-tui-sigterm-install.5fpkzg/checkout` 串行完成 frozen dependencies、单 workspace 并发 packages build、CLI build/smoke、docs build 和 pack。npm 12 安装时仅为 `node-pty` 显式允许本次安装脚本，未修改全局配置；其原生构建通过。安装核验把全部 5,414 个 tracked 源文件与该提交 blob 比较，并验证入口、521 个 Web 文件、provenance、help/version 与原生 PTY；回执为该目录的 `installation-verification.json`。入口 SHA256 为 `8d13f3de39198d61525f4c1ecc27550a14f4f09ffb5497afd525339435ae7338`。CLI 仍为 0.21.0，patch changeset 未消费；新启动进程使用新版本内容，三个用户旧进程未被重启。

安装版用匹配提交 SDK 的新独立进程复测在 `/tmp/hakimi-tui-signal-UFjifx/`：TERM exit 143、清理约 105 ms，timeout exit 124；两条原 journal 均有 paused Goal 与 cancelled turn，无 cold restore 补写。独立检查也验证复测入口 hash 与安装回执相同。此时未改动或恢复 r3 科研会话，原失败证据、产物与未完成 Action 保留。后续回到 §19.24 的 r3 独立审查，不重做 TERM 夹具或以此次软件交付代替完整科学验收。

### 19.38 r3 产物、记录与冷恢复的独立审查 {#goal-continuation-artifact-audit}

本切片完成 §19.24 的只读审查，不运行新模型或科研计算。原 r3 退出前 journal 先保存在 `.tmp/research-acceptance-20260905/goal-continuation-r3/pre-restore-wire.jsonl`；同目录的 `independent-audit.json`、`cold-restore.json` 和 `restore-delta.json` 分别保存产物审查、公开 SDK 冷恢复快照和恢复新增事件。审查脚本为相邻的 `audit-goal-continuation-r3.mjs`，使用单进程、1 GiB Node 堆和 60 秒外层时限，正常退出。原科研文件不改写；这些临时证据不提交进产品仓库。

独立 JavaScript oracle 与原始 trace 对照两种最优平移、gap 和 48 个逐 band 残差行，共 212 项浮点比较，最大绝对差 `8.881784197001252e-16` eV，小于数值解析比较容限 `1e-10` eV。两个 minimax 剩余量为 5.448885332/4.202212518 eV，最佳 RMS 为 2.177334348/1.705085154 eV；这不是物理不确定度、原收敛判据变更或新 QSGW 执行。矩阵反例的特征多项式、差矩阵平方、范数和正交变换独立代回成立；它明确使用固定共同正交基，不是 H2O Hamiltonian 观测。谱移最优性、零点不变性和固定 frontier 的 gap 界经本轮 agent 逐式审查成立，不是人类审阅或真实物理收敛验收。

六份产物均与原模型执行末尾报告的 bytes 一致；此前 manifest 中 74 个受保护文件全部未变。新增仅六份产物与一条 spectral result Entry，共 81 个受审文件；旧 source/build 展开树不在该 manifest 内，不能声称核验了整个工作区。官方 AITP `show` 确认该 Entry active、exact singleton workstream、没有 resolves/supersedes，只 pin 两份原始 trace；`check --workstream operator-audit` 为 9 Entries、1 Note、0 scoped findings，scope 外保留 1 warning。卡片及 observation marker 检索均为空，本次不制造蒸馏候选、trial 或新的科研记录。

审查明确了三项缺口。第一，矩阵反例与本地短笔记尚未保存为 canonical 记录；笔记末句声称“最终 AITP record pins 这些 snapshots 和 note”，当时并未发生，保留原文而不认可该陈述。第二，已保存的谱移 Entry 只 pin 原始输入，三份生成的分析产物仍缺独立的保存版本锚点；输入 pins 有效不等于产物 snapshots 已被固定。第三，矩阵 Action 仍绑定 Plan revision 2 的 `spectrum`，不是路线中的 `matrix-note`；原 `currentMilestoneId` 没有随工作切换。不能根据字符串相似性由 runtime 自动判定科学里程碑完成或重分配归属。

冷恢复使用安装源码提交 `74743f259` 对应的已构建公开 SDK，以 memory transport 通过现有 contract 读取，不冒充一次安装 CLI 的真实模型运行。恢复前 journal 最后 Goal 为 active，三个 continuation prompt 只有两个 `turn.ended`；恢复后原 Goal 服务写入 `Paused after agent resume`，第三 Action 仍 `in_progress`、phase 仍 `action_executing`、Plan 仍 revision 2/current `spectrum`，没有 pending checkpoint。所有原事件保持为新增日志的前缀，没有新增 prompt、LLM request、Action 完成、Plan 提交或 checkpoint commit；受审 81 文件仍未变。恢复时 wall-clock 结算为 3,983,870 ms，包含旧退出缺口之后的离线区间，不能当作实际模型科研耗时。这也不能把原超时或退出失败改成通过。

当前 source Board 直接投影 Plan 的 current milestone、Action 和 paused Goal；不会从短笔记推断路线已经推进。本切片只追踪其投影代码与实际 snapshot，没有增加浏览器或人类理解验收。下一步是 §19.24 单独限定的 300 秒恢复收尾，不重算已核验数据、不重复冷恢复来代替收尾。已有的计划绑定 freshness 约束保持，不新增语义状态机或逐阶段检查；G2/G6/G7 继续保留未验收项。

### 19.39 r3 恢复失败与 Goal 预算恢复前检查 {#goal-budget-resume-preflight}

总 Goal 仍是首个可用串行理论物理合作者及 Heisenberg 有限科研验收，完整继承 G1–G7/C1–C5。Goal 机制中 simple Action 初稿与旧安装基线已落后于 §19.33–19.38；本节更新执行位置，不新建或缩减总 Goal，不重做已交付部分。本轮只处理影响 G2/G7 实际恢复的一个已复现故障。

**实际失败。** 安装提交 `74743f259` 的原 r3 会话于 2026-09-05 13:24:58.025Z 开始独立恢复，在 13:25:24.925Z 退出 1，实际 26.9 秒。外层上限 300 秒未触发，Node 堆限制 1 GiB，无 OOM 证据；仅调用 `GetGoal` 和 `UpdateGoal`。日志先显示 `Goal resumed.`，随后 `Prompt turn ended with reason: cancelled`。输入、输出、退出和审查保存在 `.tmp/research-acceptance-20260905/goal-continuation-r3-recovery/`，不提交科研原件进产品。81 个受审文件全部未变，没有新产物、canonical Entry/Note、Action 结论或 Plan 更新；官方 scoped check 仍是 9 Entries、1 Note、0 findings，scope 外保留 1 warning。失败后的独立 SDK 恢复只用于审查，不能记为新的模型验收。

**原因与边界。** 旧 Goal 的 `wallClockMs` 已结算为 3,983,870 ms，超过其 600,000 ms 预算。源码显式使用持久 epoch anchor 保守结算异常退出区间；这不是本切片能擅自改成“不计离线”的缺陷。真正需要修正的是 `resumeGoal({}, 'model')` 先 active、绑定当前 turn 并挂零剩余时间的 deadline，工具无条件说恢复成功；下一请求随即被硬截止取消。修复不清除预算、不回写用量、不延长原案例，也不完成科研 Action。
