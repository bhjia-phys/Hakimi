...
export interface StatusReportOptions {
  ...
  /** Upstream Kimi Code baseline (version + commit); rows are omitted when absent. */
  readonly upstream?: {
    readonly version: string;
...
export function buildStatusReportLines(options: StatusReportOptions): string[] {
  ...
  const rows: FieldRow[] = [];
  // Upstream baseline provenance — informational only; never used for update
  // checks or version comparison. The commit is shown short (7 chars).
  const upstream = options.upstream;
  if (upstream !== undefined) {
    rows.push(
      { label: 'Based on', value: `Kimi Code ${upstream.version}` },
      { label: 'Upstream', value: upstream.commit.slice(0, 7) },
    );
  }
  rows.push(
