...
- `apps/kimi-web` is the browser Web UI and the only editable production source for the tracked derived outputs `apps/kimi-code/dist-web` and `apps/kimi-code/web-base.json`; use `./dev` (or `pnpm dev`) from the repository root to start the source backend plus Vite and open the browser; `./Hakimi/dev` also works from the parent directory without Corepack. The separate `dev:server` / `dev:web` commands remain available for manual setups. After source changes, regenerate both package assets with `pnpm run build:web-assets`, commit them together, then verify a clean rebuild with `pnpm run build:web-assets -- --check` — never hand-edit or partially replace the outputs; CI, native, release, direct package build/prepack, and Nix all enforce the canonical toolchain, provenance schema v5 binds source/recipe files plus the actual canonical Node/pnpm and bundle identities, the v4 native receipt directly binds that toolchain and the final binary hash, and rollback restores source/recipe from a canonical tag before regeneration.
...
- [AITP Research Protocol](https://github.com/bhjia-phys/AITP-Research-Protocol) is the source of truth for its stages, official Skills, CLI commands, schemas, and golden fixtures. Research Mode targets lightweight mode/Skill visibility only: ordinary file tools maintain project knowledge through existing project indexes, and official AITP Skills + CLI maintain long-term memory on meaningful deltas. Do not reintroduce the native adapter, `aitp_*` wrappers, host Research orchestration/vetoes, or a legacy execution mode; old records are read-only history. Mode/turn boundaries must not automatically run the CLI or write the ledger. Do not copy AITP's runtime, parser, validator, canonical ledger writes, or full Skill content.
