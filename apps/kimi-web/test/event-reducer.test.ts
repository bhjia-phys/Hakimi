...
describe('reduceAppEvent toolOutput — outputText accumulation', () => {
  function stateWithToolUse(): {
    ...
    const sessionId = 's1';
    const msgId = 'msg_tool';
    const toolCallId = 't1';
    const state = {
      ...createInitialState(),
      sessions: [makeSession(sessionId, '2026-01-01T00:00:00.000Z')],
    };
    const msg: AppMessage = {
      id: msgId,
      sessionId,
      role: 'assistant',
      content: [{ type: 'toolUse', toolCallId, toolName: 'bash', input: {} }],
      createdAt: '2026-01-01T00:00:00.000Z',
    };
    const seeded = reduceAppEvent(
      state,
      { type: 'messageCreated', message: msg },
      { sessionId, seq: 1 },
    );
    return { state: seeded, sessionId, msgId, toolCallId };
  }

  function toolUsePart(state: ReturnType<typeof createInitialState>, sessionId: string, msgId: string) {
    const part = state.messagesBySession[sessionId]?.find((m) => m.id === msgId)?.content[0];
    if (part?.type !== 'toolUse') throw new Error('expected toolUse part');
    return part;
  }

  it('appends each raw chunk verbatim into outputText while keeping outputLines', () => {
    const { state, sessionId, msgId, toolCallId } = stateWithToolUse();
    const afterFirst = reduceAppEvent(
      state,
      { type: 'toolOutput', sessionId, toolCallId, outputChunk: 'Local: http://local', stream: 'stdout' },
      { sessionId, seq: 2 },
    );
    const afterSecond = reduceAppEvent(
      afterFirst,
      { type: 'toolOutput', sessionId, toolCallId, outputChunk: 'host:5173/\n', stream: 'stdout' },
      { sessionId, seq: 3 },
    );
    const part = toolUsePart(afterSecond, sessionId, msgId);
    // outputLines keeps the chunk array untouched (UI rendering).
    expect(part.outputLines).toEqual(['Local: http://local', 'host:5173/\n']);
    // outputText is the exact no-separator concatenation — a URL split across
    // the two chunks is recoverable from it.
    expect(part.outputText).toBe('Local: http://localhost:5173/\n');
  });

  it('also accumulates for a toolUse part that already carried seeded outputText', () => {
    const sessionId = 's1';
    const msgId = 'msg_tool';
    const toolCallId = 't1';
    const state = {
      ...
      content: [
        {
          ...
          toolName: 'bash',
          input: {},
          outputLines: ['http://localhost:51'],
          outputText: 'http://localhost:51',
        },
      ],
      createdAt: '2026-01-01T00:00:00.000Z',
    };
    ...
    const next = reduceAppEvent(
      seeded,
      { type: 'toolOutput', sessionId, toolCallId, outputChunk: '73/\n', stream: 'stdout' },
      { sessionId, seq: 2 },
    );
    const part = toolUsePart(next, sessionId, msgId);
    // The seeded prefix + the new chunk reconstruct the full URL.
