...
Hakimi uses changesets for versioning. Each package gets its own `CHANGELOG.md`. The user-facing package, `@bhjia-phys/hakimi`, writes its changelog here:

```text
apps/kimi-code/CHANGELOG.md
```

This file is the **only package source** for the documentation-site changelog. Internal package changelogs such as `packages/*/CHANGELOG.md` do not go into the documentation site.

After the release flow finishes and the canonical GitHub release/tag `hakimi-v<version>` exists, maintainers manually run this skill to copy the new Hakimi changelog entries into the docs site, translate the English increment into Chinese, wait for the human review checkpoint, then optionally commit on a dedicated branch and open a PR.

## When To Use

- A new Hakimi version has a published canonical GitHub release/tag named `hakimi-v<version>`.
- `apps/kimi-code/CHANGELOG.md` contains version blocks above the first `0.13.0` baseline that are not yet at the top of `docs/en/release-notes/changelog.md`.
- The `gen-docs` flow does not run this automatically; maintainers must explicitly do it after release.

Do **not** run this before the Release PR is merged. At that point, changesets has not yet written the new version into `apps/kimi-code/CHANGELOG.md`, and the canonical GitHub release/tag does not yet prove publication.

## Source And Targets

| File | Role | Edited by |
|---|---|---|
| `apps/kimi-code/CHANGELOG.md` | **Only package source**, generated by changesets | Never edit manually |
...
- Every pending version has a canonical GitHub release/tag named exactly `hakimi-v<version>`.
- The changelog block recorded at that tag matches the current pending source block.
- The first `0.13.0` block exists as Hakimi's stop-marker/baseline. Preserve it but never backfill it into docs; only the following blocks, beginning with Kimi Code `0.36.1`, are the upstream archive, and they are not sync candidates.

GitHub release/tag state is the publication authority for this workflow. Do **not** use npm registry lookups or package-scoped tags as a fallback.

If any condition is not true, stop and confirm with the user.

Do **not** edit or commit directly on `main`. All sync work happens on a dedicated branch created in step 1, and every git mutation or outward-facing action still requires its own confirmation.

## Workflow

### 1. Prepare Branch

First use read-only commands to inspect the candidate range:

```bash
rg '^## ' apps/kimi-code/CHANGELOG.md | head -8
rg '^## ' docs/en/release-notes/changelog.md | head -5
```

If branch work is requested, start from an up-to-date default branch. Ask for explicit confirmation separately before each `git fetch`, `git checkout`, `git pull`, and branch-creation command; approval for one does not authorize the next.
...
Name the branch after the newest Hakimi version being synced:

```text
docs/changelog-sync-<newest-version>
```

Example: syncing `0.13.1` only → `docs/changelog-sync-0.13.1`.
...
Run the fail-closed detector first:

```bash
node docs/scripts/sync-changelog.mjs --dry-run
```

The source has an explicit release-line boundary:

- The **first** `## 0.13.0` block is Hakimi's stop-marker/baseline. It is not part of the upstream archive, but it is never backfilled into the docs changelog.
- Only version blocks above that stop-marker can be pending Hakimi releases.
- Only the blocks after the stop-marker, beginning with Kimi Code `0.36.1`, are the preserved upstream archive; never sync or rewrite them as Hakimi releases.
- If no block exists above the stop-marker, the sync is a no-op because the docs page already begins its preserved archive at Kimi Code `0.36.1`.
- For future releases, sync every block from the source top down to the latest already-synced Hakimi version, newest first. A pending version must still have its canonical `hakimi-v<version>` tag; the stop-marker cannot hide an untagged new release.

If the script cannot find the baseline, resolve a tag, validate tag contents, or parse a date, stop instead of choosing a range by hand.
...
- The package H1 `# @bhjia-phys/hakimi` because the docs page already has `# Changelog`.
...
Drop SDK-only and provider-internal detail. This changelog serves `@bhjia-phys/hakimi` CLI and web users. Within an entry, keep only what CLI/web users can perceive, and remove sentences that document internals instead of user-visible behavior. Apply this on both the English and Chinese pages:
...
Keep this exact Hakimi page header and archive notice:
...
Insert new Hakimi version blocks immediately after the header paragraph and before the previous latest Hakimi version or the preserved upstream archive.
...
Take the date from the version's canonical GitHub Release tag, not from when you run the sync:

```bash
git log -1 --format=%cs "hakimi-v<version>"
```

Use the half-width parenthesis form ` (YYYY-MM-DD)` on the English page. Never invent or guess a date; if the canonical tag is missing, stop and confirm with the user.
...
- Header:
  ...
  本页记录 Hakimi 版本。自 Kimi Code 0.36.1 起的版本是保留的上游历史，不属于 Hakimi 的独立发布线。
  ```

- Preserve version headings including the release date, but use full-width parentheses on the Chinese page, such as `## 0.13.1（2026-05-26）`. The date must match the English page; only the parenthesis style differs (half-width `()` in English, full-width `（）` in Chinese).
...
After verification passes, ask the user whether they want to review the sync result. Use `AskQuestion` with options such as:

- **Review first** — show the diff and wait for the user to finish checking.
- **Skip review** — move to the separate git-action confirmations in steps 9 and 10.
...
4. Do **not** stage, commit, push, or open a PR until the user explicitly says review is complete or skips review.

If the user requests edits during review, make the changes, re-run verification from step 7, and return to this checkpoint.

Review completion does not authorize later git actions. Each mutation or outward-facing action below needs a fresh confirmation immediately before it runs.

### 9. Commit

Only enter this step when the user skipped review or confirmed review is complete.

Ask for explicit confirmation immediately before staging, then stage only the changelog docs files:

```bash
git add docs/en/release-notes/changelog.md docs/zh/release-notes/changelog.md
```

Ask again immediately before committing. Use a neutral docs-sync commit message:
...
Do not run this step automatically after committing.

Ask for explicit confirmation immediately before pushing:

```bash
git push -u origin HEAD
```

After the push succeeds, ask again immediately before creating the PR with `gh pr create`. The title follows Conventional Commits:
...
- **Problem**: the docs-site changelog is behind the published Hakimi release(s).
...
The docs-site changelog has not yet been synced for `<version range>` after the canonical GitHub release.
...
- `@bhjia-phys/hakimi` and canonical `hakimi-v<version>` GitHub releases/tags define the Hakimi publication boundary.
- The first source `0.13.0` block is Hakimi's stop-marker/baseline: preserve it but never backfill it. Only subsequent blocks, starting with Kimi Code `0.36.1`, are the upstream archive, and they are not re-synced.
- Never edit `apps/kimi-code/CHANGELOG.md` manually.
- Do not backfill unreleased `.changeset/*.md` drafts into the docs site.
- If package wording is wrong, leave the package changelog alone and fix it in a future changeset.
- Always sync on a `docs/changelog-sync-*` branch and open a PR; never push changelog docs sync directly to `main`.
- Wait for the human review checkpoint, then obtain fresh confirmation before each branch, stage, commit, push, and PR action.
...
| Running branch, stage, commit, push, or PR actions under an earlier approval | Ask again immediately before each action; review completion is not standing authorization |
| Using curly quotes or half-width Chinese punctuation | Follow `docs/AGENTS.md` |
| Omitting the release date from a version heading, or guessing it | Add ` (YYYY-MM-DD)` (full-width `（）` in Chinese) taken from canonical `hakimi-v<version>` |

## Stop Signals

- A pending version has no canonical `hakimi-v<version>` GitHub release/tag, or its tagged changelog content differs.
- The first `0.13.0` stop-marker/baseline is missing, a proposed range backfills that marker, or it includes the upstream archive that follows from Kimi Code `0.36.1`.
...
- A branch, stage, commit, push, or PR action lacks fresh explicit confirmation.
