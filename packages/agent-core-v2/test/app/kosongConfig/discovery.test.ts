...
describe('refreshProviderModels write behavior', () => {
  ...
  it('preserves a legacy pool when a refresh drops its default alias', async () => {
    const baseUrl = 'https://api.managed.example.test/coding/v1';
    vi.stubEnv('KIMI_CODE_BASE_URL', baseUrl);
    const fetchMock = vi.fn(
      async () =>
  ...
  it('preserves all legacy pool entries when a refresh drops a managed alias', async () => {
    const baseUrl = 'https://api.managed.example.test/coding/v1';
    vi.stubEnv('KIMI_CODE_BASE_URL', baseUrl);
    const fetchMock = vi.fn(
      async () =>
