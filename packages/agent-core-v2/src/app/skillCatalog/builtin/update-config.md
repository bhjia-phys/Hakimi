...
- **`config.toml`** — agent / runtime settings: `default_model`, canonical `[subagent]` routes for Agent, AgentSwarm, and Tower (plus `timeout_ms`), `providers`, `models`, `thinking`, `permission`, `hooks`, `loop_control`, etc. The deprecated `[secondary_model]` section is retained for explicit config/API round-tripping and, when the legacy flag is enabled without an active preset, best-effort fallback for Agent, AgentSwarm, and Tower workers; it is not a model-pool control surface.
