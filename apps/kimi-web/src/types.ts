...
export interface ToolCall {
  ...
  /** Byte-faithful continuous transcript of the tool's streamed output —
   *  verbatim chunk concatenation (eventReducer) or '\n'-joined lines for
   *  seeded/restored output, and the raw result string once a toolResult
   *  arrives. Consumers that need the EXACT text (e.g. dev-server URL
   *  detection across chunk boundaries) must read this, never guess from
   *  `output`. */
  outputText?: string;
  media?: ToolMedia;
...
/** A local http(s) dev-server URL offered for the web preview panel — detected
 *  in a tool call's output and normalized to the loopback interface with an
 *  explicit port (see lib/devServerUrl.ts). */
export interface WebPreviewTarget {
  url: string;
}

/**
 * The SINGLE preview target of an assistant turn's standalone preview card —
 * artifact-driven first (a successfully written/edited previewable file, or
 * image/video media a tool produced), with a plain local dev-server URL as the
 * web fallback. Artifact targets win over `web`; at most one target is ever
 * produced per turn (see lib/turnWebPreview.ts).
 */
export type TurnPreviewTarget =
  | { kind: 'artifact-file'; path: string }
  | { kind: 'artifact-media'; media: ToolMedia }
  | { kind: 'web'; url: string };

/** The artifact-driven subset of TurnPreviewTarget (file / media, never web) —
 *  what ArtifactPreviewCard renders. */
export type ArtifactPreviewTarget = Extract<TurnPreviewTarget, { kind: 'artifact-file' | 'artifact-media' }>;
 ...
 *  time; opening a new one closes the previous. The authoritative definition
 *  lives in composables/useFilePreview.ts and is re-exported; keep this in
