...
export function applyOpenAICodexConfig(
  ...
  const canPreserveDefaultModel =
    options.preserveDefaultModel === true &&
    configuredDefaultModel !== undefined &&
    models[configuredDefaultModel] !== undefined;
  if (!canPreserveDefaultModel) {
...
export class OpenAICodexOAuthToolkit {
  ...
  tokenProvider(oauthRef?: OpenAICodexOAuthRef | undefined): BearerTokenProvider {
    ...
    const getRequestAuth = async (
      ...
      return {
        apiKey,
        headers: openAICodexRequestHeaders(accountId, this.userAgent),
      };
    };
    return {
...
function openAICodexRequestHeaders(
