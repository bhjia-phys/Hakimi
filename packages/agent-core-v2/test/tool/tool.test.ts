...
describe('Agent tool description', () => {
  ...
  it('hides the model parameter from the schema while the experiment is disabled', () => {
    ctx = createTestAgent();
    const tool = ctx.toolsData().find((entry) => entry.name === 'Agent');
  ...
  it('exposes the model parameter in the schema when the experiment is enabled', () => {
...
=======
  it('keeps explicit model choices ahead of active Agent routes while passing route thinking', async () => {
    const lifecycle = createAgentLifecycleStub({
      createAgentIds: ['agent-primary', 'agent-secondary', 'agent-routed'],
    });
    const context = createAgentToolContext(lifecycle, secondaryModelFlags(), {
      initialConfig: {
        ...
        subagent: {
          preset: 'research',
          agents: {
            coder: { model: 'provider/agent-route', thinkingEffort: 'agent-effort' },
          },
          presets: {
            research: {
              coder: { model: 'provider/preset-route' },
    ...
    expect(lifecycle.create).toHaveBeenNthCalledWith(
      1,
      expect.objectContaining({
        binding: expect.objectContaining({ model: 'mock-model', thinking: 'agent-effort' }),
      }),
    );
    expect(lifecycle.create).toHaveBeenNthCalledWith(
      2,
  ...
  it('rejects an explicit "secondary" request when no secondary model is configured', async () => {
