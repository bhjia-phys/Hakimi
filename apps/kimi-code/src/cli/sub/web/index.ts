/**
 * `hakimi web` — run the local Hakimi server (REST + WebSocket + web UI) in the
 * foreground and open the web UI in the default browser.
 *
 * The command itself is the runner (`hakimi web` = start the server + open the
...
export function registerWebCommand(program: Command): void {
  const web = buildWebCommand(
    program
      .command('web')
      .description('Run the local Hakimi server and open the web UI.'),
  );
