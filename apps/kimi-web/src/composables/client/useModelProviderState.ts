...
export function useModelProviderState(
  ...
  let presetMainMutationCounter = 0;
  const presetMainMutationByTarget = new Map<string, number>();

  /** Apply a preset's main route to one captured runtime target without touching
   *  global config again. The caller already saved the exact CLI-equivalent patch;
   *  reusing setModel/setThinking here would issue a second, lossy thinking write. */
  async function applyPresetMainRoute(
    route: { model?: string; thinkingEffort?: string },
    targetSessionId: string | null = rawState.activeSessionId ?? null,
  ): Promise<boolean> {
    const model = route.model;
    const thinking = route.thinkingEffort;
    if (model === undefined && thinking === undefined) return true;

    const targetKey = targetSessionId ?? '__draft__';
    const mutationId = ++presetMainMutationCounter;
    presetMainMutationByTarget.set(targetKey, mutationId);
    ...
    const previousModel = rawState.sessions.find((session) => session.id === sid)?.model;
    const previousDisplayedThinking = rawState.thinking;
    const previousSessionThinking = rawState.thinkingBySession[sid];
    if (model !== undefined) updateSession(sid, (session) => ({ ...session, model }));
    if (thinking !== undefined) {
      if (rawState.activeSessionId === sid) rawState.thinking = thinking;
      rawState.thinkingBySession = { ...rawState.thinkingBySession, [sid]: thinking };
    }

    const persisted = await persistSessionProfile({ model, thinking }, sid);
    if (persisted) return true;
    if (presetMainMutationByTarget.get(targetKey) !== mutationId) return false;

    const currentModel = rawState.sessions.find((session) => session.id === sid)?.model;
    if (model !== undefined && previousModel !== undefined && currentModel === model) {
      updateSession(sid, (session) => ({ ...session, model: previousModel }));
    }
    if (thinking !== undefined && rawState.thinkingBySession[sid] === thinking) {
      const nextThinkingBySession = { ...rawState.thinkingBySession };
      if (previousSessionThinking === undefined) delete nextThinkingBySession[sid];
      else nextThinkingBySession[sid] = previousSessionThinking;
      rawState.thinkingBySession = nextThinkingBySession;
    }
    if (
      thinking !== undefined &&
      rawState.activeSessionId === sid &&
      rawState.thinking === thinking
    ) {
      rawState.thinking = previousDisplayedThinking;
    }
    return false;
  }
