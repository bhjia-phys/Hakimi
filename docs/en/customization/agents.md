...
Hakimi includes three built-in sub-agents, ready to use out of the box, each aimed at a different task shape:
...
Hakimi discovers agent files by scope; more specific scopes take higher priority: **Explicit (`--agent-file`) > Project > Extra > User > Plugin > Built-in**. When two files define the same `name`, the higher-priority scope wins. Each directory is scanned recursively for `.md` files.

**User level** (applies to all projects):
- `$KIMI_CODE_HOME/agents/` (default: `~/.hakimi/agents/`)
- `~/.agents/agents/`

The Hakimi-specific user agent directory moves with `KIMI_CODE_HOME` (and `HAKIMI_HOME`, which takes priority), while the generic `~/.agents/agents/` directory stays under the real OS home so it can be shared across tools.
...
Hakimi Web shows each confirmed subagent's profile role and actual bound model on the `Agent` card, detail panel, background task row, and live `AgentSwarm` member row. When older history does not contain that metadata, the UI leaves it absent rather than guessing.

With experimental automatic preset switching enabled (`[subagent.auto_preset]`, see [Configuration files](../configuration/config-files.md#automatic-preset-switching)), the candidate order supplies a weighted local preference rather than a strict fallback chain. The engine combines provider quota and reset time with recent local profile/provider reliability, first-token latency, token usage, and route/model fit, then chooses the highest-scoring healthy candidate. A lower-listed preset can overtake when that evidence is materially better. Route or quota gaps, a quota below the floor, and an open circuit breaker make a candidate unavailable; a healthy current preset changes only after the winner clears the score margin and switch cooldown, while an unhealthy current preset may escape immediately.

Evaluation runs immediately before a fresh `Agent` spawn, each rebindable `Agent` resume, the new-item route and each rebindable resumed child of an `AgentSwarm` call, and a Tower worker/reviewer spawn. Profiles with `preserveBindingOnResume` skip both evaluation and rebinding. The scorer is deterministic and local: it stays fail-open, does not train a model or upload user content, and never runs as an idle background poll. In Hakimi Web, the chat-header Preset menu and **Settings > Agent** show the latest reason, candidate scores, quota, missing evidence, cooldown, circuit-breaker state, manual lock, and resume-auto control. Choosing base routing or a preset manually persists that choice and locks it against automatic changes until you resume automatic selection; `/preset auto` provides the same unlock in the TUI. Each successful automatic switch adds a localized reason marker to the triggering session.

### Selecting the Main Agent

Two CLI flags select which agent drives a new session, in both print mode (`hakimi -p`) and the interactive TUI:
