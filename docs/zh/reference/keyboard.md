...
Hakimi 的 TUI 交互模式支持一套键盘快捷键。键位按使用场景分为五组：通用输入、模式切换、流式输出期间、工具输出控制、审批面板，以及弹窗浏览。在 TUI 中输入 `/help` 可随时打开内置快捷键清单。
...
| `Ctrl-D` | 在输入框为空时退出 Hakimi |
| `Ctrl-T` | 在非 Research Mode 下，待办列表被截断时展开或折叠独立的 Todo 面板 |
...
| 快捷键 | 功能 |
| --- | --- |
| `Ctrl-O` | 展开或折叠工具输出、压缩摘要和 Research Board |

历史中存在折叠的工具调用结果时，按 `Ctrl-O` 可在折叠和展开之间切换。在 Research Mode 中，同一个快捷键会在 Todo slot 中展开或折叠 Research Board，包括研究线摘要和 Actions 投影。压缩完成后，同一个快捷键也会在压缩块中显示或隐藏压缩摘要。
...
- [hakimi 命令](./kimi-command.md) — 启动参数与子命令完整参考
