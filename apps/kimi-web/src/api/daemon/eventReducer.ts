  ...
  const next = messages.map((message) => {
    ...
    const content = message.content.map((part) => {
      ...
      return {
        ...
        // Byte-faithful continuous text: each raw chunk is appended verbatim,
        // so URLs / ANSI sequences split across chunk boundaries survive
        // intact (outputLines is kept as-is for the chunk/line UI).
        outputText: (part.outputText ?? '') + outputChunk,
      };
    });
