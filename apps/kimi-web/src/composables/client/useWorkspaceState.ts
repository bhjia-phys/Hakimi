...
export function useWorkspaceState(rawState: ExtendedState, deps: UseWorkspaceStateDeps) {
  ...
  let configMutationsInFlight = 0;
  let reconcileConfigAfterMutationFailure = false;
  ...
  async function loadConfig(): Promise<void> {
    ...
    try {
      ...
      if (
        ...
        revisionAtRequest !== configRevision ||
        configMutationsInFlight > 0
      ) {
        return;
      }
      applyConfig(next);
  ...
  async function refreshProviderUsage(providerId?: string): Promise<void> {
    const requestId = ++providerUsageRequestId;
    rawState.providerUsageLoading = true;
    rawState.providerUsageError = null;
    try {
      const api = getKimiWebApi();
      const usage: ProviderUsageResult[] = await api.getProviderUsage(providerId);
      if (requestId !== providerUsageRequestId) return;
      rawState.providerUsage = usage;
    } catch (err) {
      if (requestId !== providerUsageRequestId) return;
      rawState.providerUsageError = err instanceof Error ? err.message : String(err);
    } finally {
      if (requestId === providerUsageRequestId) {
        rawState.providerUsageLoading = false;
        rawState.providerUsageLoaded = true;
      }
    }
  }
  ...
  async function load(): Promise<void> {
    ...
    try {
      ...
      // Establish the global WS immediately after auth/config even when there
      // are no sessions, so CLI/file edits still reach this client.
