...
// ---------------------------------------------------------------------------
// Global WS connection (eager after auth/config); session subscriptions stay lazy
// ---------------------------------------------------------------------------
...
const providerUsageError = computed<string | null>(() => rawState.providerUsageError);
...
const workspaceState = useWorkspaceState(rawState, {
  ...
  nextOptimisticMsgId,
  getEventConn: () => eventConn,
  ensureEventConnection: connectEventsIfNeeded,
