...
`active` describes the Goal lifecycle; it does not claim that a model turn is running at every instant. A background-task wait lease, a paused or degraded Research Loop, or an unresolved manual Research decision can deliberately hold the next continuation. The Research Board shows that condition. In `auto` permission mode, routine in-scope Research actions use the normal tool permission policy and do not create a second Research approval gate.
