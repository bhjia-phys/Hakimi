...
Hakimi has three places where runtime parameters can be influenced: the config file, command-line options, and environment variables. They are not a simple "whoever has higher priority wins" relationship — the three serve different scenarios and have non-overlapping scopes:
...
1. **Locating the config file**: `HAKIMI_HOME` (highest priority) or `KIMI_CODE_HOME` sets the data root directory, making the config file path `$KIMI_CODE_HOME/config.toml` (or `$HAKIMI_HOME/config.toml`). This step runs before all other resolution and is not a fallback for individual parameters.
...
2. **User config file** (`~/.hakimi/config.toml`): stores long-term preferences
...
```sh
KIMI_CODE_HOME="$PWD/.hakimi-sandbox" hakimi
```
...
```sh
hakimi --yolo -p "Batch rename the following files..."
```
