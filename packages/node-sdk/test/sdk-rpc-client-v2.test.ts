...
describe('SDKRpcClientV2 Research memory mode', () => {
  it('round-trips the toggle and event without mounting the retired executor', async () => {
    const homeDir = await mkdtemp(join(tmpdir(), 'sdk-research-home-'));
    const workDir = await mkdtemp(join(tmpdir(), 'sdk-research-work-'));
    tempDirs.push(homeDir, workDir);
    const client = new SDKRpcClientV2({ homeDir, identity: TEST_IDENTITY });
    const summary = await client.createSession({ id: 'ses_research_memory', workDir });
    const session = new Session({ id: summary.id, workDir, summary, rpc: client });
    const events: Event[] = [];
    const unsubscribe = session.onEvent((event) => {
      if (event.type === 'research_mode.updated') events.push(event);
    });
    try {
      expect(await session.getResearch()).toEqual({ enabled: false, skillsAvailable: false });
      const entered = await session.commandResearch({ kind: 'enter_mode', actor: 'user' });
      expect(entered.snapshot).toEqual({ enabled: true, skillsAvailable: false });
      const agent = await ensureMainAgent(getLiveSessionById(client.engineAccessor, session.id)!);
      expect(() => agent.accessor.get(IAgentResearchService)).toThrow();
      await expect(session.commandResearch({ kind: 'pause_loop', expectedRevision: 0 })).rejects.toMatchObject({ code: 'research.retired' });
      await expect(session.commandResearch({ kind: 'discard_historical_checkpoint', checkpointId: 'old', expectedRevision: 0 })).rejects.toMatchObject({ code: 'research.retired' });
      const exited = await session.commandResearch({ kind: 'exit_mode' });
      expect(exited.snapshot.enabled).toBe(false);
      expect(events).toEqual(expect.arrayContaining([
        expect.objectContaining({ type: 'research_mode.updated', snapshot: entered.snapshot }),
        expect.objectContaining({ type: 'research_mode.updated', snapshot: exited.snapshot }),
      ]));
