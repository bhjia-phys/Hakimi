...
`active` 描述的是 Goal 生命周期，不表示每一刻都有模型轮次正在运行。默认 v2 引擎的 Goal snapshot 还可以公开后台任务 wait lease 等瞬态 continuation 细节。轻量 Research Mode 架构不增加 Research 专属的完成或续行否决，普通 Goal 行为、Plan 模式和工具权限保持不变。轻量 Research Mode 已在本地实现并安装；新启动的 Hakimi 进程使用本工作区构建产物，已在运行的进程需重新启动。它不增加 Research 专属的完成或续行否决，普通 Goal 行为、Plan 模式和工具权限保持不变。历史中关于 Research Loop、Action、alignment gate 和 Research Board 的说明不代表新后端。
