...
`,
  ...
  it('prints the sign-up page and GitHub Issues links when not signed in', async () => {
    ...
    expect(transcript).toContain('https://github.com/bhjia-phys/Hakimi/issues');
  });
  ...
  it('falls back to GitHub Issues when the sign-in status cannot be read', async () => {
    ...
    expect(openUrl).toHaveBeenCalledWith('https://github.com/bhjia-phys/Hakimi/issues');
    expect(promptFeedbackInput).not.toHaveBeenCalled();
    expect(harness.auth.submitFeedback).not.toHaveBeenCalled();
    const transcript = stripSgr(renderTranscript(driver));
  ...
  it('refreshes all supported provider models before opening /model picker', async () => {
    ...
    const refreshProviderModels = vi.spyOn(tui.authFlow, 'refreshProviderModels').mockImplementation(async () => {
      await Promise.resolve();
      tui.setAppState({
        availableModels: {
          k2: {
