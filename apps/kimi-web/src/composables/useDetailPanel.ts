...
import type { AgentMember, ToolDiffTarget, WebPreviewTarget } from '../types';
...
export function useDetailPanel({
  ...
  // ---------------------------------------------------------------------------
  // Web preview (local dev-server URLs from bash tool-call output)
  // ---------------------------------------------------------------------------
  // The target is just the normalized URL; the iframe lives in
  // WebPreviewPanel.vue and is keyed on the refresh key so reload remounts
  // ONLY the iframe (the panel shell keeps its state).
  const webPreviewTarget = ref<WebPreviewTarget | null>(null);
  // Bumped on every open/reload; WebPreviewPanel uses it as its iframe :key so
  // a refresh is a fresh navigation instead of reusing the frame's DOM.
  const webPreviewRefreshKey = ref(0);

  const webPreviewVisible = computed(() => webPreviewTarget.value !== null);

  function openWebPreview(target: WebPreviewTarget): void {
    // Clicking the same URL again toggles the panel closed.
    if (detailTarget.value === 'preview' && webPreviewTarget.value?.url === target.url) {
      closeWebPreview();
      return;
    }
    webPreviewTarget.value = target;
    webPreviewRefreshKey.value += 1;
    detailTarget.value = 'preview';
  }

  function closeWebPreview(): void {
    webPreviewTarget.value = null;
    if (detailTarget.value === 'preview') detailTarget.value = null;
  }

  function reloadWebPreview(): void {
    if (webPreviewTarget.value !== null) webPreviewRefreshKey.value += 1;
  }

  function openExternalWebPreview(): void {
    const url = webPreviewTarget.value?.url;
  ...
  const sidePanelVisible = computed(
    () =>
      ...
      (detailTarget.value !== 'preview' || webPreviewVisible.value),
  );
  ...
  // toolDiff / preview) or already stored per session (btw), we remember which
  // one was open and restore it when the user switches back.
  //
  ...
  function captureSnapshot(): PanelSnapshot | null {
    switch (detailTarget.value) {
      ...
      case 'preview':
        return webPreviewTarget.value ? { kind: 'preview', url: webPreviewTarget.value.url } : null;
      case 'btw':
        return { kind: 'btw' };
      default:
        return null;
  ...
  function restoreSnapshot(snap: PanelSnapshot | undefined): void {
    ...
    switch (snap.kind) {
      ...
      case 'toolDiff':
        ...
        break;
      case 'preview':
        webPreviewTarget.value = { url: snap.url };
        // Fresh iframe per restore (like a fresh open).
        webPreviewRefreshKey.value += 1;
        detailTarget.value = 'preview';
        break;
      case 'btw':
  ...
  function closeOpenSidePanel(): boolean {
    ...
    if (detailTarget.value === 'preview') { closeWebPreview(); return true; }
  ...
  return {
    ...
    selectDiffFile,
    webPreviewTarget,
    webPreviewRefreshKey,
    webPreviewVisible,
    openWebPreview,
    closeWebPreview,
    reloadWebPreview,
    openExternalWebPreview,
    btwVisible,
    openSideChatTab,
    closeSideChat,
    hideSideChatPanel,
