...
|---|---|---|---|
| A | Web | Hakimi in-repo production source owner | `apps/kimi-web`、桌面/浏览器 Web、Tower graph editor/live monitor，以及 schema v4 `dist-web` provenance |
...
- Web production source cutover 已完成：`apps/kimi-web` 是唯一可编辑的 production source；`apps/kimi-code/dist-web` 与 schema v4 `web-base.json` 是被 Git 跟踪的派生发布产物。源码变更后，`pnpm run build:web-assets` 从完整 source tree 按 canonical recipe/toolchain 构建并原子替换二者，提交后再用 `pnpm run build:web-assets -- --check` 做 clean rebuild 与逐字节核验；CI、release 与 native 先校验 tracked outputs 再生成；direct package build/prepack 与 Nix 从源码生成并验证结果。生成物不得手工编辑或局部替换；native receipt 绑定 source/recipe/bundle identity 与最终 binary sha256。
...
- **P2.5 Web provenance（已完成）：** A 以 `pnpm run build:web-assets` 固化 clean source build → branding patch → schema v4 provenance/verification → atomic bundle cutover；`source` 绑定完整 `apps/kimi-web` tree，`recipe` 绑定 canonical build files 与 Node/pnpm 要求，`bundle` 绑定 `dist-web` 文件清单，三者各有 sha256。`dist-web`/`web-base.json` 作为派生发布产物纳入 Git；源码变更后必须整体重新生成并一并提交，`pnpm run build:web-assets -- --check` 必须逐字节复现 tracked bundle/provenance。CI、release 与 native 先校验 tracked outputs 再生成；direct package build/prepack 与 Nix 从源码生成并验证结果；仅 Nix sandbox 可显式跳过 canonical toolchain mismatch，普通 CI 保持严格。native snapshot 再验证 bundle，receipt 绑定 source/recipe/bundle identity、branding patch 与 binary sha256。任一输入、摘要或 receipt 不匹配时 packaging 失败。回滚只允许恢复旧 canonical tag 的 source/recipe，再生成完整 bundle/provenance 与 native receipt；不得单独替换 `dist-web`。
...
- Web source 由 A 在 `apps/kimi-web` 实现；每次可视化交付都必须通过 canonical build 生成 `dist-web`，携带 schema v4 source/recipe/bundle provenance、contract fixture 和 accessibility evidence。
...
- **P3.4 visual editor/monitor：** A 在 `apps/kimi-web` 实现 template graph editor 和 live monitor source，E 冻结跨 surface interaction/accessibility contract，A 同时负责 canonical build、branding、schema v4 provenance 与 packaging；覆盖 preset overlay、keyboard/focus/screen reader/contrast、窄屏和 reconnect/catch-up。
...
- **A2 provenance/packaging（已完成）：** `pnpm run build:web-assets` 在 clean staging 中构建、branding patch、生成并验证 schema v4 source/recipe/bundle provenance，再原子替换 tracked `dist-web` 与 `web-base.json`；源码变更后必须整体重新生成并一并提交，`-- --check` 不写产物，只验证 clean rebuild 与 tracked outputs 逐字节一致。native snapshot/receipt 继续验证并绑定同一 identity 与 binary sha256。
