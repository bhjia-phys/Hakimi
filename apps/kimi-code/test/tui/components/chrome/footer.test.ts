...
describe('FooterComponent line-2 hints', () => {
  ...
  it('shows Research badge when research memory mode is enabled', () => {
    const footer = new FooterComponent({ ...appState, researchMode: true });
    const line1 = stripAnsi(footer.render(120)[0] ?? '');
    expect(line1).toContain('[Research]');
  });

  it('does not show Research badge when research memory mode is disabled', () => {
    const footer = new FooterComponent({ ...appState, researchMode: false });
    const line1 = stripAnsi(footer.render(120)[0] ?? '');
    expect(line1).not.toContain('[Research]');
  });

  it('does not show Research badge when researchMode is undefined', () => {
    const footer = new FooterComponent(appState);
    const line1 = stripAnsi(footer.render(120)[0] ?? '');
    expect(line1).not.toContain('[Research]');
  });
